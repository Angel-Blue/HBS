...
function getProduct(id) {
	return shopblock.filter((product) => {
...
function setCategoriesCount() {
	for (let i = 0; i < category.length; i++) {
		var count = shopblock.filter((product) => {
			return product.category.includes(parseInt(category[i].id));
		});
		count = count.length;
...
function setDemoDate() {
	...
	shopblock.slice(0, 4).map((item) => {
		...
		item.postdate = `${today.getDate() - 2} ${changeToMonth(
			today.getMonth()
		)}, ${today.getFullYear()}`;
		return item;
	});
...
function getFilteredproducts(
	products,
	filter = { cat: "", tag: "", searchQuery: "", priceFilter: [] }
) {
	var catgoryFilter =
		filter.cat !== undefined && filter.cat !== null && filter.cat !== "";
	var tagFilter =
		filter.tag !== undefined && filter.tag !== null && filter.tag !== "";
	var priceFilterValue =
		filter.priceFilter[0] !== undefined &&
		filter.priceFilter[1] !== undefined &&
		filter.priceFilter[0] !== null &&
		filter.priceFilter[1] != null &&
		filter.priceFilter[0] !== "" &&
		filter.priceFilter[1] !== "";
	// Category filter
	if (catgoryFilter) {
		products = products.filter((product) => {
			return (
				...
				product.category.includes(parseInt(filter.cat))
			);
		});
	}
	// Tag filter
	if (tagFilter) {
		products = products.filter((product) => {
			return (
				...
				product.tags !== null &&
				product.tags.includes(parseInt(filter.tag))
			);
		});
	}
	if (priceFilterValue) {
		products = products.filter((product) => {
			return (
				...
				product.price !== null &&
				product.price > filter.priceFilter[0] &&
				product.price <= filter.priceFilter[1]
			);
		});
	}
	return products;
