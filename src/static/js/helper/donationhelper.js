...
// Post details
function getPost(id) {
	return donationblock.filter((post) => {
...
function setCategoriesCount() {
	for (let i = 0; i < category.length; i++) {
		var count = donationblock.filter((post) => {
			return post.category.includes(parseInt(category[i].id));
		});
		count = count.length;
	...
	if (catgoryFilter) {
		posts = posts.filter((post) => {
			return (
				...
				post.category !== null &&
				post.category.includes(parseInt(filter.cat))
			);
		});
	}
	// Tag filter
	if (tagFilter) {
		posts = posts.filter((post) => {
			return (
				post.tags !== undefined &&
				post.tags !== null &&
				post.tags.includes(parseInt(filter.tag))
			);
		});
	}
	return posts;
