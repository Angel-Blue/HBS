		...
		return (
			<div className="side-bar">
				...
				<div className="main-box padding-20 trending-blog-cat mb-xl-20">
					...
					<div className="delivery-slider">
						<IonRangeSlider
							...
							min={1}
							max={1000}
							from={this.state.priceFilter[0]}
							to={this.state.priceFilter[1]}
							grid={true}
							prefix="$"
					...
					<div className="filter">
						<form>
							<label className="custom-checkbox">
								...
								<span className="checkmark" /> Goldturn{" "}
								<span className="text-light-white">(3)</span>
							</label>
							<label className="custom-checkbox">
								<input type="checkbox" name="#" />{" "}
								<span className="checkmark" /> Silverline{" "}
								<span className="text-light-white">(6)</span>
							</label>
							<label className="custom-checkbox">
								<input type="checkbox" name="#" />{" "}
								<span className="checkmark" /> Gemtuk{" "}
								<span className="text-light-white">(10)</span>
							</label>
							<label className="custom-checkbox">
								<input type="checkbox" name="#" />{" "}
								<span className="checkmark" /> Ringwing{" "}
								<span className="text-light-white">(6)</span>
							</label>
						</form>
					</div>
				...
				<div className="main-box padding-20 trending-blog-cat mb-xl-20">
					...
					<div className="filter">
						<form>
							...
							<label className="custom-radio">
								<input type="radio" name="#" /> <span className="radio" />{" "}
								Medium
							</label>
							<label className="custom-radio">
				...
				<div className="main-box padding-20 blog-tags mb-xl-20">
					...
					{tags.map((item, i) => (
						<Link to={"/shop/tag/" + item.id} key={i}>
							#{item.title}
						</Link>
					))}
				</div>
						...
						{category.map((item, i) => (
							<li className="pb-xl-20 u-line mb-xl-20" key={i}>
								<Link
									...
									{item.title}
									<span className="text-light-white fw-400">{item.count}</span>
								</Link>
							</li>
						))}
					</ul>
